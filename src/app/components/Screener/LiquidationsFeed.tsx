...
import { AssetPair, LiquidationEvent } from '@/lib/screener/types'
import { subscribeLiquidations } from '@/lib/screener/mock/feeds'
...
const LiquidationsFeed: React.FC<Props> = ({ pair }) => {
	const [events, setEvents] = useState<LiquidationEvent[]>([])

	useEffect(() => {
		// TODO(real-data): swap mock for new WebSocket(WS_BASE + '/ws/liquidations-stream/' + pair.code).
		const { seed, unsubscribe } = subscribeLiquidations(pair.code, evt => {
...
export default LiquidationsFeed
