...
const CourseDetail = () => {
	...
	const handlePromoCheck = async () => {
		...
		setPromoError(null)
		try {
			const res = await axiosInterceptor.post('/api/promocode/check/', {
				promocode,
			})
			if (res.data.valid) {
				...
				<div className='max-w-7xl mx-auto px-4'>
					...
					<div className='flex flex-col-reverse lg:flex-row justify-between gap-8 mb-16'>
						...
						<div className='lg:w-1/2'>
							...
							<p className='text-white mb-12'>{data?.description}</p>
						</div>
										...
										{data?.mine ? (
											...
											<div className='flex items-center justify-between w-full'>
												<div>
													<div className='flex items-center gap-2'>
														{data?.discount ? (
															<span className='text-white text-4xl line-through'>
																{data?.price} $
															</span>
														) : null}
