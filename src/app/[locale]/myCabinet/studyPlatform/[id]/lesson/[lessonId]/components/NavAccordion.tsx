...
const NavAccordion: React.FC<NavAccordionProps> = ({ modules }) => {
	...
	const courseId = Array.isArray(courseIdParam)
		? courseIdParam[0]
		: courseIdParam || ''
	const lessonId = Array.isArray(lessonIdParam)
		? lessonIdParam[0]
		: lessonIdParam || ''
			...
			{modules.map(module => (
				<div key={module.id} className='mb-2'>
					...
					<button
						...
						className={`flex items-center justify-between w-full pt-3 px-2 ${
							openModule === module.id ? 'bg-[#FFFFFF12] rounded-t-xl' : ''
						}`}
					>
							...
							<div
								className={`w-7 h-7 flex items-center justify-center rounded-full ${
									module.passed
										...
										: 'border-gray-500 border-[1px] text-white'
								...
								.map(lesson => {
									const LessonContent = (
										<div className='flex items-center pl-1 pt-[6px]'>
											...
											<div
												className={`w-5 h-5 flex items-center justify-center rounded-full ${lesson.passed ? 'bg-[#D2D2FF] text-[#242433]' : lesson.is_accessible ? 'border-gray-500 border-[1px] text-white' : 'bg-[#D2D2FF] text-[#242433]'} text-white shrink-0 mr-2 lg:mr-[19px]`}
											>
											...
											<span
												className={`text-[#D2D2FF font-semibold text-sm ${lesson.id === +lessonId && 'underline'} ${lesson.is_accessible ? 'hover:underline' : ''}`}
											>
												{lesson.name}
											</span>
										</div>
