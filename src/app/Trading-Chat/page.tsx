...
export default function TradingChats() {
  ...
  const results = [
    {
      ...
      icon: "/TradingChat/save-money-white.png", // Тут можна змінити іконку для кожного пункту
    },
    {
      id: 2,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10 text-white">
      <div className="mb-12">
        <div className="w-[40%] mb-4 rounded-lg p-[1px] bg-gradient-to-r from-[#6b6f8a] to-[#554e8f]">
        ...
        <div className="text-7xl font-black text-[#D2D2FF] font-sans leading-tight">
          {`«Закритий трейдинг-чат»`}
        </div>

        <div className="pt-6 w-[80%] text-3xl leading-snug">
        ...
        <div className="mt-8 mb-4 flex flex-wrap gap-6 text-xl font-bold text-[#EAB308]">
          <div className="bg-white/5 px-4 py-2 rounded-md border border-white/10">
            Перший місяць — безкоштовний
          </div>
          <div className="bg-white/5 px-4 py-2 rounded-md border border-white/10 text-white">
            Далі — $20/місяць
          </div>
        ...
        <div className="text-2xl font-bold mb-4">Для тих, хто:</div>
        <div className="mb-6">
          <ul className="flex flex-col gap-4">
            <li className="font-medium text-lg flex flex-row justify-start items-start gap-3">
        ...
        <div className="flex flex-row gap-6 items-stretch w-full pt-10">
          <button className="flex-1 bg-[#EAB308] hover:bg-[#CA8A04] text-black px-4 py-6 rounded-md font-bold text-lg uppercase tracking-wide transition-colors">
            ПОДАТИ ЗАЯВКУ
          </button>
          ...
          <div className="self-start flex-[1.5] py-6 bg-gradient-to-r from-[#6b6f8a] to-[#554e8f] rounded-lg flex items-center justify-center">
            <p className="text-white text-2xl italic font-medium text-center leading-tight px-6">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-12 w-full">
              {features.map((item) => (
                <li key={item.id} className="flex flex-row items-start gap-4">
                  <div className="flex-shrink-0 bg-[#EAB308] text-black rounded-full w-16 h-16 flex items-center justify-center font-bold text-2xl shadow-lg">
                    {item.id}
                  </div>
        ...
        <div className="flex justify-center">
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-16 gap-x-12 w-full">
            {results.map((item) => (
              <li
                key={item.id}
                className="relative bg-transparent rounded-lg border-white border-2 p-6 pt-12 flex flex-col items-center gap-3"
              >
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#0a0a0a] rounded-full w-16 h-16 border-white border-2 flex items-center justify-center">
                  <Image
                    src={item.icon}
                    alt="icon"
                    width={28}
